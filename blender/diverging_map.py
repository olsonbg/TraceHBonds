...
# Copyright:   (c) 2013
#
# Modified:    Allow even number of colors to be generated so that the white
#              control point can be placed anywhere, not just in the middle.
#              -- Brian G. Olson, 27 October 2015
#
# Modified:    Option 'quiet' added, so that no output is sent to the terminal.
#              -- Brian G. Olson, 27 October 2015
#------------------------------------------------------------------------------
...
class ColorMapCreator:
    ...
    def __init__(self, RGB1, RGB2, numColors = 33., divide = 255., 
                  method = "moreland", filename = "", quiet = False):
...
#            "For diverging colormaps odd numbers of colors are desireable!"
        ...
        if filename == "":
            for c in self.colorMap:
                print("({0:f}, {1:f}, {2:f}, 1),".format(c[0], c[1], c[2]))
        else:
...
#        print('Msh={0:f} {1:f}, {2:f}, {3:f}'.format(interp,MshMid[0], MshMid[1], MshMid[2]))
...
def main():
    ...
    # simple printout
    #colormap = ColorMapCreator(RGB1, RGB2, numColors=33)
    colormap = ColorMapCreator(RGB1, WHITE, numColors=3)
    colormap = ColorMapCreator(WHITE, RGB2, numColors=8)
    
    # save to specific file
##    import os
##    current_directory = os.getcwd()
##    fname = "RedBlueColormap.dat"
##    filename = os.path.join(current_directory, fname)
##    colormap = ColorMapCreator(RGB1, RGB2, filename = filename)
